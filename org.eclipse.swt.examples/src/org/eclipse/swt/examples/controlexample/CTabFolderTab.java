...
class CTabFolderTab extends Tab {
	...
	Button				simpleTabButton, singleTabButton, imageButton, showMinButton, showMaxButton,
			unselectedCloseButton, unselectedImageButton;
	...
	void createExampleWidgets() {
		...
		for (int i = 0; i < CTabFolderTab.CTabItems1.length; i++) {
			CTabItem item = new CTabItem(tabFolder1, SWT.NONE);
			item.setText(CTabFolderTab.CTabItems1[i]);
	...
	void hookCustomListener(final String eventName) {
		if (eventName.equals("CTabFolderEvent"))
	...
	void setItemFont() {
		...
		TableItem item = colorAndFontTable.getItem(CTabFolderTab.ITEM_FONT);
	...
	void setSelectionBackground() {
		...
		TableItem item = colorAndFontTable.getItem(CTabFolderTab.SELECTION_BACKGROUND_COLOR);
		Image oldImage = item.getImage();
		if (oldImage != null)
			oldImage.dispose();
	...
	void setSelectionForeground() {
		...
		TableItem item = colorAndFontTable.getItem(CTabFolderTab.SELECTION_FOREGROUND_COLOR);
		Image oldImage = item.getImage();
		if (oldImage != null)
			oldImage.dispose();
		item.setImage(colorImage(color));
