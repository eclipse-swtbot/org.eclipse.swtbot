...
public class FileViewer {
	...
	class ProgressDialog {
		...
		public ProgressDialog(Shell parent, int style) {
			...
			cancelButton.setText(FileViewer.getResourceString("progressDialog.cancelButton.text"));
		...
		public void setDetailFile(File file, int operation) {
			detailLabel.setText(FileViewer.getResourceString("progressDialog." + operationKeyName[operation]
	...
	private static void sortBlock(File[] files, int start, int end, File[] mergeTemp) {
		...
		for (int i = 0; i < length; ++i)
			if (x > mid || y <= end && FileViewer.compareFiles(files[x], files[y]) > 0)
				mergeTemp[i] = files[y++];
			else
				mergeTemp[i] = files[x++];
		for (int i = 0; i < length; ++i)
	...
	/**
	 * Returns a string from the resource bundle. We don't want to crash because of a missing String. Returns the key if
	 * not found.
	 */
	static String getResourceString(String key) {
		try {
			return FileViewer.resourceBundle.getString(key);
	...
	/**
	 * Returns a string from the resource bundle and binds it with the given arguments. If the key is not found, return
	 * the key.
	 */
	...
	// needs
	// to
	private File[]				deferredRefreshFiles		= null;													// to
	// defer
	// notifyRefreshFiles
	// while
	// we
	// do
	// DND
	private boolean				deferredRefreshRequested	= false;													// to
	...
	private boolean				isDragging					= false;													// if
	// this
	// app
	// is
	// dragging
	private boolean				isDropping					= false;													// if
	// this
	// app
	// is
	// dropping
	...
	private File[]				processedDropFiles			= null;													// so
	// defer
	// notifyRefreshFiles
	// while
	// we
	// do
	// DND
	private ProgressDialog		progressDialog				= null;													// progress
	// dialog
	// for
	// locally-initiated
	// operations
	...
	private final String[]		tableTitles					= new String[] {
			FileViewer.getResourceString("table.Name.title"), FileViewer.getResourceString("table.Size.title"),
	...
	private final Runnable		workerRunnable				= new Runnable() {
																public void run() {
																	...
																	workerThread = null;
																	// wake up UI thread in case it is in a modal loop
																	// awaiting thread
																	// termination
																	// (see workerStop())
																	display.wake();
	...
	private void createComboView(Composite parent, Object layoutData) {
		...
		combo.addSelectionListener(new SelectionAdapter() {
			public void widgetDefaultSelected(SelectionEvent e) {
				final String lastText = (String) combo.getData(FileViewer.COMBODATA_LASTTEXT);
				String text = combo.getText();
				if (text == null)
					return;
				if (lastText != null && lastText.equals(text))
					return;
				combo.setData(FileViewer.COMBODATA_LASTTEXT, text);
				notifySelectedDirectory(new File(text));
			}

			public void widgetSelected(SelectionEvent e) {
				final File[] roots = (File[]) combo.getData(FileViewer.COMBODATA_ROOTS);
				if (roots == null)
					return;
	...
	private void createHelpMenu(Menu parent) {
		...
		item.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				box.setMessage(FileViewer.getResourceString("dialog.About.description", new Object[] { System
						.getProperty("os.name") }));
				box.open();
			}
		});
	...
	private void createShellContents() {
		shell.setText(FileViewer.getResourceString("Title", new Object[] { "" }));
		...
		dragSource.addDragListener(new DragSourceListener() {
			...
			public void dragSetData(DragSourceEvent event) {
				...
				for (int i = 0; i < dndSelection.length; i++) {
					File file = (File) dndSelection[i].getData(FileViewer.TABLEITEMDATA_FILE);
	...
	private void createTableView(Composite parent) {
		...
		for (int i = 0; i < tableTitles.length; ++i) {
			...
			column.setWidth(FileViewer.tableWidths[i]);
		}
	...
	private DragSource createTreeDragSource(final Tree tree) {
		...
		dragSource.addDragListener(new DragSourceListener() {
			...
			public void dragSetData(DragSourceEvent event) {
				...
				sourceNames = new String[dndSelection.length];
				for (int i = 0; i < dndSelection.length; i++) {
					File file = (File) dndSelection[i].getData(FileViewer.TREEITEMDATA_FILE);
					sourceNames[i] = file.getAbsolutePath();
				}
				event.data = sourceNames;
			}
	...
	private void createTreeView(Composite parent) {
		...
		tree.addTreeListener(new TreeAdapter() {
			public void treeCollapsed(TreeEvent event) {
				...
				final Image image = (Image) item.getData(FileViewer.TREEITEMDATA_IMAGECOLLAPSED);
				if (image != null)
					item.setImage(image);
			}
			...
			public void treeExpanded(TreeEvent event) {
				final TreeItem item = (TreeItem) event.item;
				final Image image = (Image) item.getData(FileViewer.TREEITEMDATA_IMAGEEXPANDED);
				if (image != null)
					item.setImage(image);
				treeExpandItem(item);
			}
		...
		for (int i = 0; i < sourceFiles.length && !progressDialog.isCancelled(); i++) {
			...
			while (!progressDialog.isCancelled())
				if (deleteFileStructure(source))
					...
					box.setMessage(FileViewer.getResourceString("dialog.FailedDelete.description",
							new Object[] { source }));
					int button = box.open();
					if (button == SWT.ABORT)
			...
			while (!progressDialog.isCancelled()) {
				if (copyFileStructure(source, dest)) {
					...
					if (event.detail == DND.DROP_MOVE && !isDragging) {
						...
						box.setMessage(FileViewer.getResourceString("dialog.FailedCopy.description", new Object[] {
								source, dest }));
						int button = box.open();
	...
	private void treeExpandItem(TreeItem item) {
		...
		final Object stub = item.getData(FileViewer.TREEITEMDATA_STUB);
		if (stub == null)
	...
	private void treeInitFolder(TreeItem item, File folder) {
		...
		item.setData(FileViewer.TREEITEMDATA_FILE, folder);
		item.setData(FileViewer.TREEITEMDATA_IMAGEEXPANDED, iconCache.stockImages[iconCache.iconOpenFolder]);
		item.setData(FileViewer.TREEITEMDATA_IMAGECOLLAPSED, iconCache.stockImages[iconCache.iconClosedFolder]);
	}
	...
	private void treeInitVolume(TreeItem item, File volume) {
		...
		item.setData(FileViewer.TREEITEMDATA_FILE, volume);
		item.setData(FileViewer.TREEITEMDATA_IMAGEEXPANDED, iconCache.stockImages[iconCache.iconOpenDrive]);
		item.setData(FileViewer.TREEITEMDATA_IMAGECOLLAPSED, iconCache.stockImages[iconCache.iconClosedDrive]);
	}
	...
	private void treeRefresh(File[] masterFiles) {
		...
		for (int i = 0; i < items.length; ++i) {
			...
			final File itemFile = (File) item.getData(FileViewer.TREEITEMDATA_FILE);
	...
	private void treeRefreshItem(TreeItem dirItem, boolean forcePopulate) {
		final File dir = (File) dirItem.getData(FileViewer.TREEITEMDATA_FILE);

		if (!forcePopulate && !dirItem.getExpanded()) {
			// Refresh non-expanded item
			if (dirItem.getData(FileViewer.TREEITEMDATA_STUB) != null) {
				FileViewer.treeItemRemoveAll(dirItem);
				new TreeItem(dirItem, SWT.NONE); // placeholder child item to get "expand" button
				dirItem.setData(FileViewer.TREEITEMDATA_STUB, null);
			}
			return;
		}
		// Refresh expanded item
		dirItem.setData(FileViewer.TREEITEMDATA_STUB, this); // clear stub flag
	...
	private void workerAddFileDetails(final File file) {
		...
		final String dateString = FileViewer.dateFormat.format(new Date(file.lastModified()));
		...
		if (file.isDirectory()) {
			...
			sizeString = FileViewer.getResourceString("filesize.KB", new Object[] { new Long(
	...
	private void workerExecute() {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				table.removeAll();
				table.setData(FileViewer.TABLEDATA_DIR, workerStateDir);
			}
		});
		dirList = FileViewer.getDirectoryList(workerStateDir);
	...
	void doDefaultFileAction(File[] files) {
		...
		if (file.isDirectory())
			...
			if (!Program.launch(fileName)) {
				...
				dialog
						.setMessage(FileViewer.getResourceString("error.FailedLaunch.message",
								new Object[] { fileName }));
	...
	File[] getRoots() {
		...
		if (System.getProperty("os.name").indexOf("Windows") != -1) {
			...
			list.add(new File(FileViewer.DRIVE_A));
			list.add(new File(FileViewer.DRIVE_B));
			for (char i = 'c'; i <= 'z'; ++i) {
	...
	void handleDeferredRefresh() {
		...
		if (files == null) {
			...
			final File[] comboRoots = (File[]) combo.getData(FileViewer.COMBODATA_ROOTS);
			...
			if (refreshCombo) {
				combo.removeAll();
				combo.setData(FileViewer.COMBODATA_ROOTS, roots);
				for (int i = 0; i < roots.length; ++i) {
	...
	void notifySelectedDirectory(File dir) {
		 ...
		 */
		shell.setText(FileViewer.getResourceString("Title", new Object[] { currentDirectory.getPath() }));
