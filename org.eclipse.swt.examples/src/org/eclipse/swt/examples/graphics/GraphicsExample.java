...
/**
 * This class is the main class of the graphics application. Various "tabs" are created and made visible by this class.
 */
public class GraphicsExample {
	...
	static String getResourceString(String key) {
		try {
			return GraphicsExample.RESOURCE_BUNDLE.getString(key);
	...
	// they appear in
	// the tree
	boolean				animate	= true; // whether animation
	// should happen

	GraphicsBackground	background;	// used to store
	// information about the
	// background
	ToolItem			backItem, dbItem;	// background,
	// double buffer
	// items
	Menu				backMenu;			// background menu
	// item
	...
	Sash				hSash, vSash;
	Composite			parent;
	ArrayList			resources;			// stores resources
	// in the
	// application
	...
	// tabs
	Text				tabDesc;			// multi-line text

	// contains backItem
	// and dbItem
	Tree				tabList;			// tree structure of

	GraphicsTab[]		tabs;				// tabs to be found

	// that will be
	// disposed
	ArrayList			tabs_in_order;		// stores

	ToolBar				toolBar;			// toolbar that
	...
	public void setTab(GraphicsTab tab) {
		...
		if (children.length != 0)
			...
			data.top = new FormAttachment(100, -GraphicsExample.MARGIN);
	...
	boolean checkAdvancedGraphics() {
		...
		try {
			...
			dialog.setText(GraphicsExample.RESOURCE_BUNDLE.getString("Warning")); //$NON-NLS-1$
			dialog.setMessage(GraphicsExample.RESOURCE_BUNDLE.getString("LibNotFound")); //$NON-NLS-1$
			dialog.open();
			if (newShell != null)
				newShell.dispose();
			return false;
		}
		return GraphicsExample.advanceGraphics = true;
	}
	...
	void createControlPanel(Composite parent) {
		...
		group.setText(GraphicsExample.getResourceString("Settings")); //$NON-NLS-1$
	...
	void createControls(final Composite parent) {
		...
		data = new FormData();
		data.left = new FormAttachment(0, GraphicsExample.MARGIN);
		data.top = new FormAttachment(0, GraphicsExample.MARGIN);
		data.right = new FormAttachment(100, -GraphicsExample.MARGIN);
		toolBar.setLayoutData(data);

		data = new FormData();
		data.left = new FormAttachment(0, GraphicsExample.MARGIN);
		data.top = new FormAttachment(toolBar, GraphicsExample.MARGIN);
		data.right = new FormAttachment(vSash, -GraphicsExample.SASH_SPACING);
		data.bottom = new FormAttachment(hSash, -GraphicsExample.SASH_SPACING);
		...
		data = new FormData();
		data.left = new FormAttachment(0, GraphicsExample.MARGIN);
		data.top = new FormAttachment(hSash, GraphicsExample.SASH_SPACING);
		data.right = new FormAttachment(vSash, -GraphicsExample.SASH_SPACING);
		data.bottom = new FormAttachment(100, -GraphicsExample.MARGIN);
		...
		data = new FormData();
		data.left = new FormAttachment(vSash, GraphicsExample.SASH_SPACING);
		data.top = new FormAttachment(toolBar, GraphicsExample.MARGIN);
		data.right = new FormAttachment(100, -GraphicsExample.MARGIN);
	...
	GraphicsTab[] createTabs() {
		return new GraphicsTab[] { new LineTab(this), new StarPolyTab(this), tab = new IntroTab(this),
				new BlackHoleTab(this), new AlphaTab(this), new BallTab(this), new CountDownTab(this),
				new CurvesSWTTab(this), new CurvesTab(this), new CustomFontTab(this), new FontBounceTab(this),
				new GradientTab(this), new ImageTransformTab(this), new ShapesTab(this), new MazeTab(this),
				new RGBTab(this), new SpiralTab(this), new CardsTab(this), new LineCapTab(this),
				new InterpolationTab(this), new PathClippingTab(this), new PathClippingAnimTab(this),
				new LineStyleTab(this), new LineJoinTab(this), new RegionClippingTab(this), new CustomAlphaTab(this),
				new TextAntialiasTab(this), new GraphicAntialiasTab(this), new ImageFlipTab(this), new PathTab(this), };
	}
	...
	void createToolBar(final Composite parent) {
		...
		back.setText(GraphicsExample.getResourceString("Back")); //$NON-NLS-1$
		...
		next.setText(GraphicsExample.getResourceString("Next")); //$NON-NLS-1$
		...
		backItem.setText(GraphicsExample.getResourceString("Background")); //$NON-NLS-1$
		...
		dbItem.setText(GraphicsExample.getResourceString("DoubleBuffer")); //$NON-NLS-1$
