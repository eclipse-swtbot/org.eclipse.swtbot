...
/**
 * This example demonstrates how to implement hover help feedback using the MouseTrackListener.
 */
public class HoverHelp {
	/**
	 * Emulated tooltip handler Notice that we could display anything in a tooltip besides text and images. For
	 * instance, it might make sense to embed large tables of data or buttons linking data under inspection to material
	...
	public String getResourceString(String key) {
		try {
			return HoverHelp.resourceBundle.getString(key);
		...
		try {
			if (images == null) {
				...
				for (int i = 0; i < HoverHelp.imageLocations.length; ++i) {
					InputStream stream = clazz.getResourceAsStream(HoverHelp.imageLocations[i]);
