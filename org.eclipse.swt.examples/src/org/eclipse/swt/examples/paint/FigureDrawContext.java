...
public class FigureDrawContext {
	/*
	 * <p> The GC must be set up as follows (it will be returned to this state upon completion of drawing operations)
	 * <ul> <li>setXORMode(false) </ul> </p>
	 */
	public GC	gc	= null;
	...
	public Rectangle toClientRectangle(int x1, int y1, int x2, int y2) {
		return new Rectangle(Math.min(x1, x2) * xScale - xOffset, Math.min(y1, y2) * yScale - yOffset, (Math.abs(x2
				- x1) + 1)
				* xScale, (Math.abs(y2 - y1) + 1) * yScale);
	}
}
