...
class SashTab extends Tab {
	...
	void createExampleWidgets() {
		...
		list1.setItems(SashTab.ListData0);
		list2 = new List(sashComp, SWT.V_SCROLL | SWT.H_SCROLL | SWT.BORDER);
		list2.setItems(SashTab.ListData1);
		...
		hSash.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				event.y = Math.min(Math.max(event.y, SashTab.SASH_LIMIT), rect.height - SashTab.SASH_LIMIT);
		...
		vSash.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				Rectangle rect = vSash.getParent().getClientArea();
				event.x = Math.min(Math.max(event.x, SashTab.SASH_LIMIT), rect.width - SashTab.SASH_LIMIT);
				if (event.detail != SWT.DRAG) {
	...
	/**
	 * Layout the list and text widgets according to the new positions of the sashes..events.SelectionEvent
	 */
	void layout() {
		...
		list2.setBounds(vSashBounds.x + vSashBounds.width, 0, clientArea.width - (vSashBounds.x + vSashBounds.width),
				hSashBounds.y);
		text.setBounds(0, hSashBounds.y + hSashBounds.height, clientArea.width, clientArea.height
				- (hSashBounds.y + hSashBounds.height));

		/**
		 * If the horizontal sash has been moved then the vertical sash is either too long or too short and its size
		 * must be adjusted.
		 */
	...
	void resized() {
		...
		/*
		 * Make list 1 half the width and half the height of the tab leaving room for the sash. Place list 1 in the top
		 * left quadrant of the tab.
		 */
		Rectangle list1Bounds = new Rectangle(0, 0, (clientArea.width - SashTab.SASH_WIDTH) / 2,
				(clientArea.height - SashTab.SASH_WIDTH) / 2);
		list1.setBounds(list1Bounds);

		/*
		 * Make list 2 half the width and half the height of the tab leaving room for the sash. Place list 2 in the top
		 * right quadrant of the tab.
		 */
		list2.setBounds(list1Bounds.width + SashTab.SASH_WIDTH, 0, clientArea.width
				- (list1Bounds.width + SashTab.SASH_WIDTH), list1Bounds.height);

		/*
		 * Make the text area the full width and half the height of the tab leaving room for the sash. Place the text
		 * area in the bottom half of the tab.
		 */
		text.setBounds(0, list1Bounds.height + SashTab.SASH_WIDTH, clientArea.width, clientArea.height
				- (list1Bounds.height + SashTab.SASH_WIDTH));

		/* Position the sashes */
		vSash.setBounds(list1Bounds.width, 0, SashTab.SASH_WIDTH, list1Bounds.height);
		hSash.setBounds(0, list1Bounds.height, clientArea.width, SashTab.SASH_WIDTH);
	}
