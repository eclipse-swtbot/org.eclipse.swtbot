...
public class StarPolyTab extends GraphicsTab {
	...
	public StarPolyTab(GraphicsExample example) {
		super(example);
		radial = new int[StarPolyTab.POINTS * 2];
	}
	...
	public void paint(GC gc, int width, int height) {
		...
		int pos = 0;
		for (int i = 0; i < StarPolyTab.POINTS; ++i) {
			double r = Math.PI * 2 * pos / StarPolyTab.POINTS;
			radial[i * 2] = (int) ((1 + Math.cos(r)) * centerX);
			radial[i * 2 + 1] = (int) ((1 + Math.sin(r)) * centerY);
			pos = (pos + StarPolyTab.POINTS / 2) % StarPolyTab.POINTS;
		}
