...
/**
 * AddressBookExample is an example that uses <code>org.eclipse.swt</code> libraries to implement a simple address
 * book. This application has save, load, sorting, and searching functions common to basic address books.
 */
public class AddressBook {
	...
	private static final String[]	columnNames		= { AddressBook.resAddressBook.getString("Last_name"),
			AddressBook.resAddressBook.getString("First_name"), AddressBook.resAddressBook.getString("Business_phone"),
			AddressBook.resAddressBook.getString("Home_phone"), AddressBook.resAddressBook.getString("Email"),
			AddressBook.resAddressBook.getString("Fax") };
	...
	public void open(Shell shell) {
		...
		searchDialog.setSearchAreaNames(AddressBook.columnNames);
		searchDialog.setSearchAreaLabel(AddressBook.resAddressBook.getString("Column"));
		...
		for (int i = 0; i < AddressBook.columnNames.length; i++) {
			TableColumn column = new TableColumn(table, SWT.NONE);
			column.setText(AddressBook.columnNames[i]);
			column.setWidth(150);
	...
	private boolean closeAddressBook() {
		if (isModified) {
			...
			box.setMessage(AddressBook.resAddressBook.getString("Close_save"));
	...
	private MenuItem createEditMenu(Menu menuBar) {
		...
		item.setText(AddressBook.resAddressBook.getString("Edit_menu_title"));
		...
		subItem.setText(AddressBook.resAddressBook.getString("Edit"));
		...
		subItem.setText(AddressBook.resAddressBook.getString("Copy"));
		...
		// Edit -> Paste
		subItem = new MenuItem(menu, SWT.NONE);
		subItem.setText(AddressBook.resAddressBook.getString("Paste"));
		...
		// Edit -> Delete
		subItem = new MenuItem(menu, SWT.NONE);
		subItem.setText(AddressBook.resAddressBook.getString("Delete"));
		subItem.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				TableItem[] items = table.getSelection();
				if (items.length == 0)
		...
		subItem.setText(AddressBook.resAddressBook.getString("Sort"));
	...
	private void createFileMenu(Menu menuBar) {
		// File menu.
		MenuItem item = new MenuItem(menuBar, SWT.CASCADE);
		item.setText(AddressBook.resAddressBook.getString("File_menu_title"));
		Menu menu = new Menu(shell, SWT.DROP_DOWN);
		item.setMenu(menu);
		/**
		...
		subItem.setText(AddressBook.resAddressBook.getString("New_contact"));
		...
		subItem = new MenuItem(menu, SWT.NONE);
		subItem.setText(AddressBook.resAddressBook.getString("Edit_contact"));
		subItem.setAccelerator(SWT.MOD1 + 'E');
		subItem.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				TableItem[] items = table.getSelection();
		...
		subItem = new MenuItem(menu, SWT.NONE);
		subItem.setText(AddressBook.resAddressBook.getString("New_address_book"));
		...
		// File -> Open
		subItem = new MenuItem(menu, SWT.NONE);
		subItem.setText(AddressBook.resAddressBook.getString("Open_address_book"));
		...
		// File -> Save.
		subItem = new MenuItem(menu, SWT.NONE);
		subItem.setText(AddressBook.resAddressBook.getString("Save_address_book"));
		...
		// File -> Save As.
		subItem = new MenuItem(menu, SWT.NONE);
		subItem.setText(AddressBook.resAddressBook.getString("Save_book_as"));
		...
		// File -> Exit.
		subItem = new MenuItem(menu, SWT.NONE);
		subItem.setText(AddressBook.resAddressBook.getString("Exit"));
		subItem.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				shell.close();
			}
	...
	private void createHelpMenu(Menu menuBar) {
		...
		// Help Menu
		MenuItem item = new MenuItem(menuBar, SWT.CASCADE);
		item.setText(AddressBook.resAddressBook.getString("Help_menu_title"));
		Menu menu = new Menu(shell, SWT.DROP_DOWN);
		item.setMenu(menu);
		...
		subItem.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				box.setText(AddressBook.resAddressBook.getString("About_1") + shell.getText());
				box.setMessage(shell.getText() + AddressBook.resAddressBook.getString("About_2"));
				box.open();
			}
		});
	}
	...
	private Menu createPopUpMenu() {
		...
		item.setText(AddressBook.resAddressBook.getString("Pop_up_new"));
		...
		item.setText(AddressBook.resAddressBook.getString("Pop_up_edit"));
		item.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				TableItem[] items = table.getSelection();
				if (items.length == 0)
		...
		// Copy
		item = new MenuItem(popUpMenu, SWT.PUSH);
		item.setText(AddressBook.resAddressBook.getString("Pop_up_copy"));
		item.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				TableItem[] items = table.getSelection();
				if (items.length == 0)
		...
		// Paste
		item = new MenuItem(popUpMenu, SWT.PUSH);
		item.setText(AddressBook.resAddressBook.getString("Pop_up_paste"));
		item.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				if (copyBuffer == null)
					return;
	...
	private void createSearchMenu(Menu menuBar) {
		// Search menu.
		MenuItem item = new MenuItem(menuBar, SWT.CASCADE);
		item.setText(AddressBook.resAddressBook.getString("Search_menu_title"));
		Menu searchMenu = new Menu(shell, SWT.DROP_DOWN);
		item.setMenu(searchMenu);

		// Search -> Find...
		item = new MenuItem(searchMenu, SWT.NONE);
		item.setText(AddressBook.resAddressBook.getString("Find"));
		...
		// Search -> Find Next
		item = new MenuItem(searchMenu, SWT.NONE);
		item.setText(AddressBook.resAddressBook.getString("Find_next"));
	...
	private Menu createSortMenu() {
		...
		for (int i = 0; i < AddressBook.columnNames.length; i++) {
			...
			subitem.setText(AddressBook.columnNames[i]);
			final int column = i;
			subitem.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
					sort(column);
	...
	private String[] decodeLine(String line) {
		...
		for (int i = 0; i < parsedLine.length - 1; i++) {
			int index = line.indexOf(AddressBook.DELIMITER);
			if (index > -1) {
				parsedLine[i] = line.substring(0, index);
				line = line.substring(index + AddressBook.DELIMITER.length(), line.length());
			} else
				return null;
		}

		if (line.indexOf(AddressBook.DELIMITER) != -1)
			return null;
	...
	private void editEntry(TableItem item) {
		...
		dialog.setLabels(AddressBook.columnNames);
	...
	private boolean findMatch(String searchString, TableItem item, int column, boolean matchWord, boolean matchCase) {
		...
		} else if (tableText != null && tableText.indexOf(searchString) != -1)
			return true;
		return false;
	}

	private void newAddressBook() {
		shell.setText(AddressBook.resAddressBook.getString("Title_bar")
				+ AddressBook.resAddressBook.getString("New_title"));
		file = null;
		isModified = false;
	}
	 ...
	 * @param data
	 */
	public void addAddressBook(String[] data) {
		if (data != null) {
			TableItem item = new TableItem(table, SWT.NONE);
			item.setText(data);
			isModified = true;
	...
	private void openAddressBook() {
		...
		fileDialog.setFilterNames(new String[] { AddressBook.resAddressBook.getString("Book_filter_name") + " (*.adr)",
				AddressBook.resAddressBook.getString("All_filter_name") + " (*.*)" });
		...
		if (!file.exists()) {
			displayError(AddressBook.resAddressBook.getString("File") + file.getName() + " "
					+ AddressBook.resAddressBook.getString("Does_not_exist"));
			return;
		}
		...
		try {
			...
			displayError(AddressBook.resAddressBook.getString("File_not_found") + "\n" + file.getName());
			return;
		} catch (IOException e) {
			...
			if (fileReader != null)
				try {
					...
					displayError(AddressBook.resAddressBook.getString("IO_error_close") + "\n" + file.getName());
					return;
				}
		}
		...
		shell.setText(AddressBook.resAddressBook.getString("Title_bar") + fileDialog.getFileName());
		isModified = false;
		this.file = file;
	}
	...
	private boolean save() {
		...
		shell.setText(AddressBook.resAddressBook.getString("Title_bar") + file.getName());
		isModified = false;
		return true;
	}
	...
	private boolean saveAs() {
		...
		if (file.exists()) {
			...
			box.setText(AddressBook.resAddressBook.getString("Save_as_title"));
			box.setMessage(AddressBook.resAddressBook.getString("File") + file.getName() + " "
					+ AddressBook.resAddressBook.getString("Query_overwrite"));
			if (box.open() != SWT.YES)
				return false;
		}
		this.file = file;
