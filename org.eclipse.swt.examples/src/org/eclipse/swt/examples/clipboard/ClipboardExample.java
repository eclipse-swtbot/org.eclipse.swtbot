...
public class ClipboardExample {
	...
	public void open(Shell parentShell) {
		parentShell.setText("SWT Clipboard");
		parentShell.setLayout(new FillLayout());

		ScrolledComposite sc = new ScrolledComposite(parentShell, SWT.H_SCROLL | SWT.V_SCROLL);
		...
		parentShell.setSize(Math.min(size.x, monitorArea.width - 20), Math.min(size.y, monitorArea.height - 20));
		parentShell.open();
	}
	...
	void createControlTransfer(Composite parent) {
		...
		data.heightHint = data.widthHint = ClipboardExample.SIZE;
