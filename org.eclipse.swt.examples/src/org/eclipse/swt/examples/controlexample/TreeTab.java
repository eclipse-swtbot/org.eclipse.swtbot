...
class TreeTab extends ScrollableTab {
	...
	Button				multipleColumns, moveableColumns, resizableColumns, headerVisibleButton, sortIndicatorButton,
			headerImagesButton, subImagesButton, linesVisibleButton;
	...
	Group				treeGroup, imageTreeGroup, checkedTreeGroup, itemGroup;
	...
	void createExampleGroup() {
		...
		/* Create a group for the checked tree */
		checkedTreeGroup = new Group(exampleGroup, SWT.NONE);
		checkedTreeGroup.setLayout(new GridLayout());
		checkedTreeGroup.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
		checkedTreeGroup.setText(ControlExample.getResourceString("Tree_Checked"));
	}
	...
	void createExampleWidgets() {
		...
		if (multiColumn) {
			for (int i = 0; i < TreeTab.columnTitles.length; i++) {
				...
				treeColumn.setText(TreeTab.columnTitles[i]);
				treeColumn.setToolTipText(ControlExample.getResourceString("Tooltip",
						new String[] { TreeTab.columnTitles[i] }));
			}
		...
		for (int i = 0; i < 4; i++) {
			...
			if (multiColumn && subImagesButton.getSelection())
				for (int j = 0; j < TreeTab.columnTitles.length; j++)
		...
		tree3 = new Tree(checkedTreeGroup, SWT.CHECK);
		for (boolean grayed : Arrays.asList(true, false)) {
			TreeItem checkedItem = new TreeItem(tree3, SWT.NONE);
			checkedItem.setGrayed(grayed);
			checkedItem.setText("grayed? " + grayed);
		}
		packColumns(tree3);
	}
	...
	Item[] getExampleWidgetItems() {
		/*
		 * Note: We do not bother collecting the tree items because tree items don't have any events. If events are ever
		 * added to TreeItem, then this needs to change.
		 */
	...
	/**
	 * Sets the initial sort indicator state and adds a listener to cycle through sort states and columns.
	 */
	 ...
	 * sets foreground and background color of the Node 1 TreeItems to default settings.
	 */
	...
	void setCellBackground() {
		...
		TableItem item = colorAndFontTable.getItem(TreeTab.CELL_BACKGROUND_COLOR);
	...
	void setCellFont() {
		...
		TableItem item = colorAndFontTable.getItem(TreeTab.CELL_FONT);
		Image oldImage = item.getImage();
		if (oldImage != null)
			oldImage.dispose();
	...
	void setCellForeground() {
		...
		TableItem item = colorAndFontTable.getItem(TreeTab.CELL_FOREGROUND_COLOR);
		Image oldImage = item.getImage();
		if (oldImage != null)
			oldImage.dispose();
		item.setImage(colorImage(color));
	...
	void setItemBackground() {
		...
		TableItem item = colorAndFontTable.getItem(TreeTab.ITEM_BACKGROUND_COLOR);
		Image oldImage = item.getImage();
		if (oldImage != null)
			oldImage.dispose();
		item.setImage(colorImage(color));
	...
	void setItemFont() {
		...
		TableItem item = colorAndFontTable.getItem(TreeTab.ITEM_FONT);
		Image oldImage = item.getImage();
		if (oldImage != null)
			oldImage.dispose();
		item.setImage(fontImage(ft));
	...
	void setItemForeground() {
		...
		TableItem item = colorAndFontTable.getItem(TreeTab.ITEM_FOREGROUND_COLOR);
		Image oldImage = item.getImage();
		if (oldImage != null)
			oldImage.dispose();
		item.setImage(colorImage(color));
	...
	void setItemText(TreeItem item, int i, String node) {
		...
		if (multipleColumns.getSelection()) {
			...
			item.setText(TreeTab.tableData[index]);
		} else
			item.setText(node);
	}
