...
class CoolBarTab extends Tab {
	/**
	 * Listens to widgetSelected() events on SWT.DROP_DOWN type CoolItems and opens/closes a menu when appropriate.
	 */
	class CoolItemSelectionListener extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent event) {
			/**
			 * A selection event will be fired when the cool item is selected by its gripper or if the drop down arrow
			 * (or 'chevron') is selected. Examine the event detail to determine where the widget was selected.
			 */
			if (event.detail == SWT.ARROW) {
				/*
				 * If the popup menu is already up (i.e. user pressed arrow twice), then dispose it.
				 */
				if (menu != null) {
					menu.dispose();
					menu = null;
				...
				/*
				 * Display the pop-up menu at the lower left corner of the arrow button. Dispose the menu when the user
				 * is done with it.
				 */
	...
	class DropDownSelectionListener extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent event) {
			...
			if (event.detail == SWT.ARROW) {
				...
				/*
				 * Main area of drop down tool item selected. An application would invoke the code to perform the action
				 * for the tool item.
				 */
			}
		}
