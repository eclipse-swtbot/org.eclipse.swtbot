...
public class PaintSurface {
		...
		paintCanvas.addPaintListener(new PaintListener() {
			public void paintControl(PaintEvent event) {
				if (rubberband.isEmpty())
					...
					/*
					 * Avoid flicker when merging overlayed objects by constructing the image on a backbuffer first,
					 * then blitting it to the screen.
					 */
	 ...
	 * <p>
	 * Increments by one the rubberband "hide" nesting count. The rubberband is hidden from view (but remains active) if
	...
	public void setStatusCoordRange(Point a, Point b) {
		statusCoordInfo = PaintExample.getResourceString("status.CoordRange.format", new Object[] { new Integer(a.x),
	 ...
	 * <p>
	 * Decrements by one the rubberband "hide" nesting count. The rubberband is only made visible when showRubberband()
	 * has been called once for each previous hideRubberband(). It is not permitted to call showRubberband() if the
	 * rubber band is not presently hidden.
	 * </p>
	 */
	...
	private void updateStatus() {
		statusText.setText(PaintExample.getResourceString("status.Bar.format", new Object[] { statusActionInfo,
