...
/**
 * This class utilizes the factory design pattern to create menus that may contain color items, pattern items and
 * gradient items. To create a menu: first set the menu items that you wish to have appear in the menu by calling the
 * setters (setColorItems(), setPatternItems(), setGradientItems()), and then call createMenu() to get an instance of a
 * menu. By default, the menu will contain color items.
 */
public class ColorMenu {
		...
		public void handleEvent(Event event) {
			...
			case SWT.Selection:
				...
				if (customColorMI == item) {
					...
					if (customImage != null)
						customImage.dispose();
					customImage = GraphicsExample.createImage(display, colorA, colorB, 16, 16);
					GraphicsBackground gb = new GraphicsBackground();
					gb.setBgImage(customImage);
					gb.setThumbNail(customImage);
	...
	/**
	 * Creates and returns the menu based on the settings provided via setColorItems(), setPatternItems() and
	 * setGradientItems()
	 * 
	...
	/**
	 * Method used to specify whether or not the color menu items will appear in the menu.
	 * 
	 * @param enable
	 *            A boolean flag - true to make the color menu items visible in the menu; false otherwise.
	 */
	...
	/**
	 * Method used to specify whether or not the gradient menu items will appear in the menu.
	 * 
	 * @param enable
	 *            A boolean flag - true to make the gradient menu items visible in the menu; false otherwise.
	 */
	...
	/**
	 * Method used to specify whether or not the pattern menu items will appear in the menu.
	 * 
	 * @param enable
	 *            A boolean flag - true to make the pattern menu items visible in the menu; false otherwise.
	 */
	...
	private void addColorItems(Menu menu, MenuItemListener menuListener, ArrayList menuResources) {
		...
		// color names
		String[] names = new String[] { GraphicsExample.getResourceString("White"), //$NON-NLS-1$
		...
		Color[] colors = new Color[] { display.getSystemColor(SWT.COLOR_WHITE),
				...
				display.getSystemColor(SWT.COLOR_YELLOW), display.getSystemColor(SWT.COLOR_CYAN), };
	...
	private void addPatternItems(Menu menu, MenuItemListener menuListener, ArrayList menuResources) {
		Display display = menu.getDisplay();
		...
		// pattern names
		String[] names = new String[] { GraphicsExample.getResourceString("Pattern1"), //$NON-NLS-1$
				GraphicsExample.getResourceString("Pattern2"), //$NON-NLS-1$
				GraphicsExample.getResourceString("Pattern3"), //$NON-NLS-1$
		};

		// pattern images
		Image[] images = new Image[] { loadImage(display, "pattern1.jpg", menuResources),
				loadImage(display, "pattern2.jpg", menuResources), loadImage(display, "pattern3.jpg", menuResources), };
