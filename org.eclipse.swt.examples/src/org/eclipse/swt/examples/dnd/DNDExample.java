...
public class DNDExample {
	...
	public Composite	defaultParent;
	...
	private void createDragWidget(Composite parent) {
		...
		combo.setItems(new String[] { "Toggle Button", "Radio Button", "Checkbox", "Canvas", "Label", "List", "Table",
				"Tree", "Text", "StyledText" });
		combo.select(DNDExample.LABEL);
	...
	private void createDropTarget() {
		...
		dropTarget.addDropListener(new DropTargetListener() {
			...
			public void drop(DropTargetEvent event) {
				...
				if ((strings.length == 1)
						&& ((dropControlType == DNDExample.TABLE) || (dropControlType == DNDExample.TREE) || (dropControlType == DNDExample.LIST))) {
				...
				case TREE: {
					...
					for (int i = 0; i < strings.length; i++) {
						TreeItem item = parentItem != null ? new TreeItem(parentItem, SWT.NONE) : new TreeItem(tree,
								SWT.NONE);
